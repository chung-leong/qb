...
static zend_always_inline const char * qb_get_zend_op_name(qb_php_translater_context *cxt, uint32_t opcode) {
...
static uint32_t ZEND_FASTCALL qb_get_zend_array_dimension_count(qb_php_translater_context *cxt, zval *zvalue, qb_primitive_type element_type) {
...
static void ZEND_FASTCALL qb_get_zend_array_dimensions(qb_php_translater_context *cxt, zval *zvalue, qb_primitive_type element_type, uint32_t *dimensions, uint32_t dimension_count) {
	if((Z_TYPE_P(zvalue) == IS_ARRAY || Z_TYPE_P(zvalue) == IS_CONSTANT_ARRAY)) {
		HashTable *ht = Z_ARRVAL_P(zvalue);
		Bucket *p;
...
static void ZEND_FASTCALL qb_copy_element_from_zval(qb_php_translater_context *cxt, zval *zvalue, qb_address *address) {
...
static void ZEND_FASTCALL qb_copy_elements_from_zend_array(qb_php_translater_context *cxt, zval *zvalue, qb_address *address) {
...
static qb_address * ZEND_FASTCALL qb_obtain_zval_constant(qb_php_translater_context *cxt, zval *zvalue, qb_primitive_type desired_type) {
...
static uint32_t ZEND_FASTCALL qb_get_zval_type(qb_php_translater_context *cxt, zval *zvalue, uint32_t flags);

static uint32_t ZEND_FASTCALL qb_get_zval_array_type(qb_php_translater_context *cxt, zval *array, uint32_t flags) {
...
static void ZEND_FASTCALL qb_translate_basic_op(qb_php_translater_context *cxt, void *op_factory, qb_operand *operands, uint32_t operand_count, qb_operand *result, qb_result_prototype *result_prototype) {
	qb_produce_op(cxt, op_factory, operands, operand_count, result, NULL, 0, result_prototype);
}
...
static void ZEND_FASTCALL qb_translate_jump(qb_php_translater_context *cxt, void *op_factory, qb_operand *operands, uint32_t operand_count, qb_operand *result, qb_result_prototype *result_prototype) {
	...
	qb_produce_op(cxt, op_factory, operands, operand_count, result, &target_index, 1, result_prototype);
...
static void ZEND_FASTCALL qb_translate_branch(qb_php_translater_context *cxt, void *op_factory, qb_operand *operands, uint32_t operand_count, qb_operand *result, qb_result_prototype *result_prototype) {
	...
	qb_produce_op(cxt, op_factory, operands, operand_count, result, target_indices, 2, result_prototype);
...
static qb_php_op_translator op_translators[] = {
	...
	{	qb_translate_basic_op,				NULL						},	// ZEND_ASSIGN_OBJ
...
static qb_intrinsic_function * ZEND_FASTCALL qb_find_intrinsic_function(qb_php_translater_context *cxt, zval *callable) {
