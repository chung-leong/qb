...
class QBInvertMatrixHandler extends QBMatrixHandler {
	...
	public function getActionForUnitData() {
		...
		if($this->operandSize == "variable") {
			...
			$lines[] = "for(m = 0, p = 0; m < dim; m++) {";
			$lines[] = 		"sign = sign_init;";
			$lines[] = 		"sign_init = -sign_init;";
			$lines[] = 		"for(n = 0; n < dim; n++) {";
			$lines[] = 			"$cType minor_det;";
			$lines[] = 			"for(i = 0, k = 0, q = 0; i < dim; i++) {";
			...
			$lines[] = 				"}";
			$lines[] = 			"}";
			$lines[] = 			"qb_calculate_cm_matrix_determinant_$type(minor, NULL, MATRIX1_COLS - 1, &minor_det, NULL);";
