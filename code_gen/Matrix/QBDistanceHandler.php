...
class QBDistanceHandler extends QBSIMDHandler {
	...
	public function getAction() {
		...
		if($this->operandSize == "variable") {
			if($this->addressMode == "ARR") {
				return "qb_calculate_array_distance_$type(op1_ptr, op1_ptr + op1_count, op2_ptr, op2_ptr + op2_count, MATRIX2_ROWS, res_ptr, res_ptr + res_count);";
			} else {
				return "qb_calculate_array_distance_$type(op1_ptr, NULL, op2_ptr, NULL, MATRIX2_ROWS, res_ptr, NULL);";
			}
		} else {
			if($this->addressMode == "ARR") {
				return "qb_calculate_array_distance_{$this->operandSize}x_$type(op1_ptr, op1_ptr + op1_count, op2_ptr, op2_ptr + op2_count, res_ptr, res_ptr + res_count);";
			} else {
