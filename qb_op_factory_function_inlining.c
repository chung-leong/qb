...
static void qb_set_writable_scalar_substitution(qb_compiler_context *cxt, qb_compiler_context *other_cxt, qb_address *other_address, qb_operand *arguments, uint32_t argument_count, qb_address_substitution *substitution) {
	qb_address *address;
...
static void qb_inline_function(qb_compiler_context *cxt, void *factory, qb_operand *operands, uint32_t operand_count, qb_operand *result, uint32_t *jump_target_indices, uint32_t jump_target_count, qb_result_prototype *result_prototype) {
	...
	for(i = 0; i < other_cxt->writable_scalar_count; i++, j++) {
		...
		qb_set_writable_scalar_substitution(cxt, other_cxt, other_address, arguments, argument_count, &substitutions[j]);
	}
