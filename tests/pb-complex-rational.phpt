...
/**
 * @engine qb
 * @import pbj/complex-rational.pbj
 *
 * @param image			$dst
 * @param image			$src
 * @param float32[2]	$a
 ...
 * @param float32[2]	$c
 * @param float32[2]	$d
 * @param float32[2]	$size
 * @param float32[2]	$center
 */
function filter(&$dst, $src, $a, $b, $c, $d, $size, $center) {}

qb_compile();

filter($output, $image, array(0.8, 0.6), array(-110, 22), array(0, 0), array(62, 34), array(390, 290), array(160, 200));
