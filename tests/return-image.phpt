...
$correct_path = "$folder/output/return.correct.png";
$incorrect_path = "$folder/output/return.incorrect.png";

/**
 * @engine qb
 * @param float32[4] $color
 * @local image		 $img
 * @return image
 */
function test_function($color) {
	array_resize($img, 30, 40);
	$img = $color;
	return $img;
} 

$image = test_function(array(0.5, 0, 0, 0.5));
