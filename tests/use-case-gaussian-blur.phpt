...
$image = imagecreatefrompng("$folder/input/fontmap.png");
$output = imagecreatetruecolor(imagesx($image), imagesy($image));
$correct_path = "$folder/output/image-gaussian-blur.correct.png";
$incorrect_path = "$folder/output/image-gaussian-blur.incorrect.png";
...
function blur(&$output, $image) {
	...
	for($iy = 0, $y = 0.5; $iy < $height; $iy++, $y++) {
		for($ix = 0, $x = 0.5; $ix < $width; $ix++, $x++) {
			$output[$iy][$ix] = sample_convolution($image, $x, $y, $gaussian, 16, 0);
		}
	}
}
