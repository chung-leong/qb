--TEST--
Pixel Bender test: Radial Caleidoscope
--SKIPIF--
<?php 
...
$filter_name = "radial-caleidoscope";
...
/**
 * @engine qb
 * @import pbj/radial-caleidoscope.pbj
 *
 * @param image			$dst
 * @param image			$src
 * @param float32		$angle
 * @param float32		$direction
 * @param float32[2]	$basepoint
 */
function filter(&$dst, $src, $angle, $direction, $basepoint) {
}

qb_compile();

filter($output, $image, 0.45, 0, array(190, 70));
