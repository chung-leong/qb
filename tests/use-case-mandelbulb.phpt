...
class Mandelbulb {
	...
	/** @var int[x,y] 		The output size in pixels. */
	public $size = array(640, 480);
	...
	{
		$direction[0] = 2.0 * $this->aspectRatio * $p->x / $this->size->x - $this->aspectRatio;
		$direction[1] = -2.0 * $p->y / $this->size->y + 1.0;
		$direction[2] = -2.0 * exp($this->cameraZoom);
	...
	protected function renderPixel($pixel)
	{
		...
		if ($this->intersectBoundingSphere($this->eye, $rayDirection, $tmin, $tmax)) {
			...
			$max_steps = $stepLimit / $this->epsilonScale;
	...
	{
		$this->aspectRatio = $this->size->x / $this->size->y;
		...
		for ($x = 0, $coord->x = 0.5; $x < $width; $x++, $coord[0]++) {
			$c = array(0, 0, 0, 0);
