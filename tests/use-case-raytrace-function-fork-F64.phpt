--TEST--
Use case: Ray Tracer (helper-function, multicore, 64-bit)
--EXTENSIONS--
gd
--SKIPIF--
<?php 
...
class RayTracer {
	...
	const MAX_RAY_SHOTS = 4;
 	...
 	/** @var float64[NUM_SPHERES][x, y, z] */
	public $spherePositions;
    
	/** @var float64[NUM_SPHERES] */
	public $sphereRadii;
    
	/** @var float64[NUM_SPHERES][r, g, b] */
	public $sphereColors;
    
	/** @var float64[NUM_SPHERES][ambient, diffuse, specular, reflectivity] */
	...
	*/
	 ...
	 * @engine qb
	 *
	 * @param float64[x, y, z]	$origin
	 * @param float64[x, y, z]	$dir
	 * @param bool				$hit
	 * @param float64[3]		$pos
	 * @param float64			$t
	 ...
	 *
	 * @local uint32			$i
	 * @local float64[3]		$sphereToOrigin
	 * @local float64			$B
	 * @local float64			$C
	 * @local float64			$disc
	 * @local float64 			$curT
	 ...
	 * @local uint32			$y	 
	 * @local float64[3]		$dst
	 * @local float64[x, y, z]	$origin
	 * @local float64[x, y, z]	$dir
	 * @local float64[3]		$sphereHit		hit point relative to sphere
	 * @local float64[3]		$n				surface normal
	 * @local float64[3]		$lightVector	surface to light
	 * @local float64			$lightVectorLen
	 * @local float64[3]		$l				normalized light vector
	 * @local float64[3]		$lReflect		reflected off surface
	 * @local float64[3]		$dirReflect
	 * @local float64[3]		$colorScale
	 * @local float64[3]		$sphereColor
	 * @local float64[ambient, diffuse, specular, reflectivity]	$sphereMaterial
	 * @local float64			$specular
	 * @local float64			$diffuse
	 * @local float64			$lightVal
	 * @local bool				$hit
	 * @local float64[3]		$hitPoint
	 * @local float64			$t
		...
		for($x = 0; $x < $width; $x++) {
			...
			while($rayShots > 0 ) {
				...
				if($hit) {
					...
					$lightVal = $sphereMaterial->ambient + float64($shadowTest) * (($diffuse * $sphereMaterial->diffuse) + ($specular * $sphereMaterial->specular));
