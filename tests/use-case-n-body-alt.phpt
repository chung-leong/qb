	...
	do {
		for($k = 0; $k < count($bodies); $k++) {
			for($m = $k + 1; $m < count($bodies); $m++) {
				$d = array_slice($bodies[$k], 0, 3) - array_slice($bodies[$m], 0, 3);
				...
				array_splice($bodies[$k], 3, 3, array_slice($bodies[$k], 3, 3) - $d * $mag_b); 
				array_splice($bodies[$m], 3, 3, array_slice($bodies[$m], 3, 3) + $d * $mag_a); 
			}
		}
		for($j = 0; $j < count($bodies); $j++) {
			array_splice($bodies[$j], 0, 3, array_slice($bodies[$j], 0, 3) + array_slice($bodies[$j], 3, 3) * 0.01);
		}
	} while(++$i < $n);
}
