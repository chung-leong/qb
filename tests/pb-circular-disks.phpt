--TEST--
Pixel Bender test: Circular Disks
--SKIPIF--
<?php 
...
/**
 * @engine qb
 * @import pbj/circular-disks.pbj
 *
 * @param image			$dst
 * @param image			$src
 * @param float32		$size
 * @param float32		$radius
 * @param float32[2]	$base
 */
function filter(&$dst, $src, $size, $radius, $base) {
}

qb_compile();

filter($output, $image, 6, 0.45, array(340, 100));
